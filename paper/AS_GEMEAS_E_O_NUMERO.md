...
| ✅ | Camada condicional: a camada errada passa a custar zero | portão deduzido, **zero bits**, prosa volta a 3,487 |
| ❌ | O espaço rende mais fora do fluxo | **−13,70%** — ele já é o símbolo mais barato |
...
*Estes números medem cada leitura **ligada à força**. A §4.4 mostra que uma camada opcional não precisa ficar ligada — e que, com portão, a que perde passa a custar zero.*
...
### 4.4 A camada que fica de sobreaviso — e nunca viaja

Correção registrada, e ela muda a arquitetura sem mudar a medida: **a camada vertical não é descartada por ter perdido em prosa. Ela fica de sobreaviso.** Só entra se precisar — e **nunca viaja**.

Isso é possível porque uma camada não é um dado, é um cálculo. Os dois lados a computam a partir do que já têm. O que precisa ser decidido não é *o que mandar*, é **quem assina a probabilidade de cada bloco**. Três portões:

| portão | quem decide | custo em bits |
|---|---|---|
| fixo | ninguém: sempre ligada, ou sempre desligada | zero |
| oráculo | o emissor olha o bloco e avisa | 1 bit por bloco |
| **deduzido** | os dois olham quem venceu o bloco **anterior** | **zero** |

O portão deduzido é admissível pela lei da §4: ele depende só do que já foi decodificado. E as duas camadas continuam se atualizando com o bit verdadeiro — quem assinou não muda o que se aprende —, então as gêmeas seguem idênticas.

**Medido** (E13):

| prosa — 9.228 bytes | bits/byte | vertical ligada em |
|---|---|---|
| camada desligada (só frente) | 3,487 | 0% dos blocos |
| vertical sempre ligada | 3,567 | 100% |
| oráculo, bloco de 64 | 3,499 | 10% |
| **deduzido, bloco de 64** | **3,495** | 10% |
| **deduzido, bloco de 1024** | **3,487** | **0%** |

| tabular — 14.037 bytes | bits/byte | vertical ligada em |
|---|---|---|
| camada desligada (só frente) | 1,137 | 0% |
| vertical sempre ligada | 0,700 | 100% |
| oráculo, bloco de 256 | 0,704 | 98% |
| **deduzido, bloco de 256** | **0,708** | 98% |

**Três leituras, e a terceira é a melhor:**

1. **O prejuízo desapareceu.** Em prosa, com bloco de 1024, o portão deduzido dá **3,487** — exatamente o custo de não ter a camada. A camada ficou de sobreaviso, nunca foi chamada, e não cobrou nada. Os −2,81% de §4.1 eram o preço de deixá-la ligada à força.
2. **O ganho ficou.** Em tabular, o mesmo portão entrega 0,708 contra 0,700 da camada sempre ligada: guarda praticamente todos os 37,71%, ligando-a em 98% dos blocos. **Um só codec, sem ajuste manual, se comporta certo nos dois tipos de dado.**
3. **O portão que não viaja ganha do portão que viaja.** Em prosa com bloco de 64, o oráculo custa **3,499** — pior que não ter camada nenhuma —, porque 1 bit por bloco são 0,0125 bits/byte, mais do que a escolha perfeita economiza. O deduzido, a **zero bits**, dá 3,495. **Avisar custou mais do que acertar rendeu.**

Isto não reabilita A7: a camada vertical continua perdendo em prosa. Mas muda o que isso significa para o projeto — **com portão, a camada errada custa zero; sem portão, custa 2,81%.** E generaliza: toda camada opcional deve nascer com portão, inclusive as três do CLX (§4.3) e as duas de §4.2. O pedágio do fatiamento só se paga onde ele rende.

### 4.5 O vácuo: o espaço carrega informação?

A proposta: o espaço não é vazio. Ele carrega — talvez semântica, talvez a indicação de uma operação, talvez uma mudança de rota — e o outro lado, sendo uma inteligência, sabe disso.

São três perguntas diferentes disfarçadas de uma. Cada uma tem uma medida, e elas não dão a mesma resposta.

**(a) Quanto o espaço custa hoje?**

**Medido** (E14), sobre a prosa do projeto:

| classe | símbolos | bits por símbolo | % do fluxo |
|---|---|---|---|
| letra | 7.136 | 3,590 | 80,0% |
| pontuação | 364 | **5,965** | 6,8% |
| **espaço** | 1.261 | **1,622** | 6,4% |
| dígito | 229 | 5,254 | 3,8% |
| quebra de linha | 238 | 4,188 | 3,1% |

**O espaço já é o símbolo mais barato do fluxo** — custa menos da metade de uma letra. O modelo o prevê muito bem, porque as letras anteriores já dizem quase tudo sobre onde a palavra acaba. Não há reserva escondida ali: mesmo que o espaço passasse a ser literalmente grátis, o teto do ganho seria 6,4%.

E a tabela aponta para outro lugar, que ninguém tinha olhado: **a pontuação é o símbolo mais caro do texto**, 5,965 bits — mais que a letra, mais que o dígito. Trezentos e sessenta e quatro símbolos consomem 6,8% do fluxo. Se há folga em algum lugar deste corpus, está na pontuação, não no espaço.

**(b) E se o espaço não viajasse, e fosse remontado no fim?**

É a forma mais forte da ideia: tira-se o espaço, manda-se só as letras, e a inteligência do outro lado repõe as separações. Testável.

| esquema | bits |
|---|---|
| espaço dentro do fluxo | 32.034 |
| texto sem espaço | 31.179 |
| camada de comprimentos de palavra | 5.244 |
| **soma das duas** | **36.423** |

**REFUTADO: −13,70%.** Tirar o espaço do fluxo sai **mais caro**, e o prejuízo é duplo. O espaço, dentro do fluxo, custava só 2.045 bits. Tirá-lo não devolveu esses 2.045 (o texto sem espaço custa 31.179, não 29.989): sem as separações, **as letras ficam mais difíceis de prever**. E a camada de comprimentos, sozinha, custa 5.244 — mais que o dobro do que o espaço custava embutido.

A lição é a mesma de §4.2, agora com outro rosto: **a informação do espaço e a informação das letras são a mesma informação.** Separar as duas paga duas vezes pela mesma coisa.

**(c) O espaço como mudança de rota**

Esta parte da proposta está certa — e é a mais interessante das três, porque **já está acontecendo**.

| o que a posição dentro da palavra diz sobre a letra | IM líquida |
|---|---|
| **marginal** — o sinal sozinho | **0,2620** |
| **condicional** — sabendo apenas **1** caractere anterior | **0,1090** |
| condicional — sabendo 4 (o que o codec já usa) | nada mensurável |

E no codec, com controle de mesmo tamanho:

| leitura | bits/byte |
|---|---|
| frente 1–4 | 3,487 |
| controle: frente 1–8 | 3,469 |
| frente 1–4 + distância desde o espaço | 3,567 |

Ganho da rota explícita sobre o controle: **−2,81%**. Nenhum.

**O espaço já é a mudança de rota, e o modelo lê isso sozinho.** Ele está dentro da janela de contexto: ver os quatro caracteres anteriores é ver o espaço, se ele estiver lá. A informação é real — 0,262 bit, medida com controle. Ela só já estava sendo usada.

**E aqui está a armadilha que este experimento documenta, que vale mais que o resultado:**

> A informação mútua **marginal** mede o que um sinal diz **sozinho**. Ela não mede o que ele **acrescenta**. A posição dentro da palavra diz 0,262 bit sozinha; sabendo um único caractere anterior, já cai para 0,109; sabendo quatro, não sobra nada que se possa medir. Empilhar sinais pela IM marginal é somar por inteiro a conta de três pessoas que dividiram a mesma refeição.

É a mesma armadilha da paridade em §6-bis.3, um andar acima: lá o controle separou sinal de ruído; aqui ele separa sinal **novo** de sinal **repetido**. Toda camada candidata precisa passar pelos dois.

**O que sobra da ideia do vácuo, e não é pouco.** A medida diz que o espaço *deste* corpus já está aproveitado. Mas há uma versão da proposta que este experimento **não** testa e que continua de pé: o **vácuo não-semântico** — espaço duplo, espaço no fim da linha, recuo, onde a linha quebra. Isso é diagramação, não conteúdo, e o fluxo paga por reproduzi-lo exatamente. Neste corpus, as quebras de linha custam **3,1% do fluxo**, a 4,188 bits cada — mais caro, por símbolo, que o próprio espaço. Normalizar a diagramação libera isso inteiro.

A forma honesta da ideia é, portanto, invertida: **o vácuo não-semântico não é um lugar para guardar informação. É desperdício — e desperdício se corta, não se enriquece.** O que é diagramação vira camada terminal de verdade (calculada no fim, nunca transmitida); o que é conteúdo já está pago.
...
| A33 | Um portão condicional zera o custo de uma camada inútil | **MEDIDO** | E13: prosa 3,487 deduzido = só-frente | uma camada que atrapalhe mesmo desligada |
| A34 | O portão deduzido (zero bits) bate o portão avisado | **MEDIDO** | E13: 3,495 contra 3,499, bloco de 64 | blocos grandes, onde o aviso dilui |
...
| só entra se precisar, e nunca viaja | portão condicional deduzido | §4.4 |
| processada no último ponto, no final | camada terminal | §4.4, §4.5 |
| o vácuo carrega informação | custo por classe de símbolo; IM condicional | §4.5 |
| o espaço é uma mudança de rota | troca de contexto — já embutida no modelo | §4.5 |
